...
- Change the way the compiler is used: Instead of loading it from the
  program, execute it with the program file as an argument, as an
  ordinary compiler. This seems the simplest way to get the path of
  the program file, in order to reuse it with the target files.
  Besides, this makes the configuration of the compiler and the
  building tools, like GNU Make and apenwarr/redo, simpler.
